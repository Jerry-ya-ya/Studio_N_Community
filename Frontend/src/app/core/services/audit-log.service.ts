...
export class AuditLogService {
  ...
  getActivityLogs(limit = 50): Observable<AuditLogResponse> {
    return this.getLogs('/admin/logs/activity', limit);
  }

  getAccountLogs(limit = 50): Observable<AuditLogResponse> {
  ...
  getTodoActionLogs(limit = 50): Observable<AuditLogResponse> {
    return this.getLogs('/superadmin/logs/todo-action', limit);
  }
