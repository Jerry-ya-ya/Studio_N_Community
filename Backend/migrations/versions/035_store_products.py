...
def upgrade():
    ...
    if 'store_product' not in inspector.get_table_names():
        op.create_table(
            'store_product',
    ...
    index_names = {
        index['name'] for index in sa.inspect(bind).get_indexes('store_product')
    }
    if 'ix_store_product_is_published' not in index_names:
        op.create_index(
    ...
    if 'ix_store_product_created_by_id' not in index_names:
        op.create_index(
...
def downgrade():
    bind = op.get_bind()
    if 'store_product' not in sa.inspect(bind).get_table_names():
        return
    index_names = {
        index['name'] for index in sa.inspect(bind).get_indexes('store_product')
    }
    if 'ix_store_product_created_by_id' in index_names:
        op.drop_index('ix_store_product_created_by_id', table_name='store_product')
    if 'ix_store_product_is_published' in index_names:
