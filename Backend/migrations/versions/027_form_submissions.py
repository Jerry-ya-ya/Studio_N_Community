...
Revision ID: 027_form_submissions
Revises: 026_form_templates
...
revision = '027_form_submissions'
down_revision = '026_form_templates'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if 'form_submission' in inspector.get_table_names():
        index_names = {index['name'] for index in inspector.get_indexes('form_submission')}
        if 'ix_form_submission_form_id' not in index_names:
            op.create_index(
                'ix_form_submission_form_id',
                'form_submission',
                ['form_id'],
                unique=False,
            )
        if 'ix_form_submission_user_id' not in index_names:
            op.create_index(
                'ix_form_submission_user_id',
                'form_submission',
                ['user_id'],
                unique=False,
            )
...
def downgrade():
    ...
    if 'form_submission' in inspector.get_table_names():
        index_names = {index['name'] for index in inspector.get_indexes('form_submission')}
        if 'ix_form_submission_user_id' in index_names:
            op.drop_index('ix_form_submission_user_id', table_name='form_submission')
        if 'ix_form_submission_form_id' in index_names:
