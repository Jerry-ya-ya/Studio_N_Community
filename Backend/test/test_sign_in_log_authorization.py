...
def test_sign_in_logs_are_only_available_to_superadmins(app, client, monkeypatch):
    unique_id = uuid4().hex

    with app.app_context():
        admin = User(
            username=f'sign-in-log-admin-{unique_id}',
            password='hashed-password',
            email=f'admin-{unique_id}@example.com',
            role='admin',
        )
        superadmin = User(
            username=f'sign-in-log-superadmin-{unique_id}',
            password='hashed-password',
    ...
    assert allowed_response.get_json()['type'] == 'sign-in'
